...
public class UserHelper : IUserHelper
{
    ...
    public UserHelper(DataContext dbContext
    ...
    public async Task AddUserClaims(TypeUser TypeUser, string email)
    {
        var usuario = await _userManager.FindByEmailAsync(email);
        await _userManager.AddClaimAsync(usuario!, new Claim(TypeUser.ToString(), "1"));
    }

    public async Task RemoveUserClaims(TypeUser TypeUser, string email)
    {
        var usuario = await _userManager.FindByEmailAsync(email);
        await _userManager.RemoveClaimAsync(usuario!, new Claim(TypeUser.ToString(), "1"));
    }
    ...
    public async Task<User> AddUserAsync(string firstname, string lastname,
            ...
            int Idcorporate, string ImagenFull, string Origin, bool UserActivo, TypeUser TypeUser)
    {
        ...
        await AddUserToRoleAsync(newUser, TypeUser.ToString());
        await AddUserClaims(TypeUser, email);
        return newUser;
    }

    public async Task<User> AddUserSoftAsync(string firstname, string lastname,
    string email, string phone, string address, string job,
